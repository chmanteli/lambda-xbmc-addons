...
class shows:
    def __init__(self):
        self.list = []
        self.data = ''
        self.skaiUrl			= 'http://www.skai.gr/player/tv'
        self.skaiUrl2			= 'http://www.skai.gr/ajax.aspx?m=Skai.Player.ItemView&type=TV'
    ...
    def skai_list(self):
        ...
        for i in range(0, len(self.list)):
            threads.append(Thread(self.skai_list2, self.list[i]['url'], i))
        [i.start() for i in threads]
        [i.join() for i in threads]

        self.list = sorted(self.list, key=itemgetter('name'))
        filter = [i for i in self.list if not i['image'] == '']
        self.list = filter
        return self.list
    ...
    def skai_list2(self, url, i):
        try:
            ...
            image = common.parseDOM(result, "Photo1")[0]
...
class episodes:
    def __init__(self):
        ...
        self.skaiUrl			= 'http://www.skai.gr/player/tv'
        self.skaiUrl2			= 'http://www.skai.gr/ajax.aspx?m=Skai.Player.ItemView&type=TV'
        self.skairecentUrl		= 'http://www.skai.gr/ajax.aspx?m=Skai.Player.ItemView&type=TV&cid=0'
        self.skainewsUrl		= 'http://www.skai.gr/ajax.aspx?m=Skai.Player.ItemView&type=TV&alid=43505&cid=6'
        self.skaisportsUrl		= 'http://www.skai.gr/ajax.aspx?m=Skai.Player.ItemView&type=TV&alid=14&cid=6'
    ...
    def skai_recent(self):
        self.list = self.skai_list('SKAI TV', self.skairecentUrl)
        index().episodeList(self.list[:100])

    def skai_news(self):
        self.list = self.skai_list('SKAI TV', self.skainewsUrl)
        index().episodeList(self.list[:100])

    def skai_sports(self):
        self.list = self.skai_list('SKAI TV', self.skaisportsUrl)
        index().episodeList(self.list[:100])
