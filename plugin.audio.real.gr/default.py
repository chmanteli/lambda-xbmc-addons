    ...
    def episodeList(self, episodeList):
        ...
        for i in episodeList:
            try:
                ...
                sysname, sysshow, sysurl, sysimage = urllib.quote_plus(name), urllib.quote_plus(show), urllib.quote_plus(url), urllib.quote_plus(image)
                u = '%s?action=parts&show=%s&url=%s' % (sys.argv[0], sysshow, sysurl)
                ...
                item = xbmcgui.ListItem(name, iconImage="DefaultFolder.png", thumbnailImage=image)
                item.setInfo( type="Music", infoLabels={ "Title": name, "Label": name, "Album": name, "Artist": show, "Genre": addonGenre } )
                item.setProperty("Album_Description", addonDesc)
                count = count + 1
                if count % 2 == 0:
                    item.setProperty("Fanart_Image", addonFanart2)
...
class shows:
    def __init__(self):
        self.list = []
        self.realUrl			= 'http://www.real.gr'
        self.realshowsUrl		= 'http://www.real.gr/DefaultArthro.aspx?page=radioathens'
        self.realepisodeUrl		= 'http://www.real.gr/DefaultArthro.aspx?page=category&getPaging=true&catid='
...
class episodes:
    ...
    def get(self, show, url):
        self.list = self.real_list(show, url)
        index().episodeList(self.list)

    def parts(self, show, url):
        self.list = self.real_parts(show, url)
        index().episodepartsList(self.list)

    def real_list(self, show, url):
        threads = []
        for i in range(1, 5):
            self.data.append('')
            episodesUrl = '%s&curPage=%s' % (url, str(i))
            threads.append(Thread(self.real_list2, episodesUrl, i))
