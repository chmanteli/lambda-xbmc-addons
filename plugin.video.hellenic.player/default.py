    ...
    def cinegreece_list(self, url):
        ...
        for show in shows:
            try:
                ...
                if name.endswith('(œ)'.decode('iso-8859-7')): name = 'œ '.decode('iso-8859-7') + name.replace('(œ)'.decode('iso-8859-7'), '').strip()
                elif name.endswith('(«)'.decode('iso-8859-7')): name = '« '.decode('iso-8859-7') + name.replace('(«)'.decode('iso-8859-7'), '').strip()
                elif name.endswith('(‘Ô)'.decode('iso-8859-7')): name = '‘Ô '.decode('iso-8859-7') + name.replace('(‘Ô)'.decode('iso-8859-7'), '').strip()
                elif name.endswith('(œÈ)'.decode('iso-8859-7')): name = 'œÈ '.decode('iso-8859-7') + name.replace('(œÈ)'.decode('iso-8859-7'), '').strip()
                elif name.endswith('(‘·)'.decode('iso-8859-7')): name = '‘· '.decode('iso-8859-7') + name.replace('(‘·)'.decode('iso-8859-7'), '').strip()
                name = common.replaceHTMLCodes(name)
                name = name.encode('utf-8')
                url = common.parseDOM(show, "a", ret="href")[0]
    ...
    def nickelodeon_list(self, name, url, image, imdb, genre, plot, show):
        ...
        for episode in episodes:
            try:
                ...
                self.list.append({'name': name, 'url': url, 'image': image, 'imdb': imdb, 'genre': genre, 'plot': plot, 'title': name, 'show': show, 'season': '1', 'episode': '1', 'sort': sort})
            except:
                pass
    ...
    def cinegreece_list(self, name, url, image, imdb, genre, plot, show):
        ...
        for episode in episodes:
            try:
                ...
                name = '≈ÂÈÛ¸‰ÈÔ '.decode('iso-8859-7') + name
                name = common.replaceHTMLCodes(name)
                name = name.encode('utf-8')
...
class player:
    ...
    def antenna(self, url):
        ...
        try:
            proxyip, proxyport = getSetting("proxyip"), getSetting("proxyport")
            proxies = ['%s:%s' % (proxyip, proxyport)]
            if proxyip == '' or proxyport == '': proxies = []
