...
class main:
    def __init__(self):
        ...
        elif action == 'startv_shows':				shows().startv()
        elif action == 'episodes':					episodes().get(show, url)
        elif action == 'episodes_recent':			episodes().startv_recent()
        elif action == 'episodes_news':				episodes().startv_news()
...
class categories:
    def get(self):
        ...
        catList.append({'name': 30502, 'image': 'Shows.png', 'action': 'startv_shows'})
...
class shows:
    ...
    def startv(self):
        channel =	'startvgr'
        filters =	["FLwUNbp_4Y2Ry-asyerw2jew", #Αγαπημένα βίντεο
					"PLCg0cLoEdHTzVUK_-OIcsj7I5yE0XmY55", #YouTube Launch Event
					"PLCg0cLoEdHTxyJ1PpAo5HDQh0RDOLoC2Q"] #Starολογία!
...
class episodes:
    ...
    def startv_recent(self):
        channel = 'startvgr'
        self.list = self.youtube_list('STAR TV', self.youtube_recentUrl % channel)
        index().episodeList(self.list[:100])

    def startv_news(self):
        playlist = 'PLCg0cLoEdHTzRWyYwABV5prVUruZGrGCl'
        self.list = self.youtube_list('STAR TV', self.youtube_episodeUrl % playlist)
        index().episodeList(self.list[:50])
